# 🎯 Actividades en Innsbruck

**Fechas tentativas:** 23 septiembre — **tránsito / 0 noches** (puede pasar a 1–2 noches)  
**Categoría:** Alpes + ciudad chica  
**Price Level:** $$$  
**Temperatura:** 6-16°C  

---

## 📍 Innsbruck ciudad (imprescindibles)

- [ ] **Altstadt (Ciudad vieja)** — calles medievales, paseo corto y muy agradable
- [ ] **Golden Roof (Goldenes Dachl)** — símbolo del centro histórico
- [ ] **Río Inn + puente** — vistas clásicas con montañas de fondo
- [ ] **Hofkirche** — tumba de Maximiliano I (si les interesa historia Habsburgo)

---

## 🏔️ Montaña (elegir 1 según clima)

- [ ] **Nordkette / Seegrube** — teleférico desde la ciudad, mirador alpino inmediato
- [ ] **Patscherkofel** — otra opción de montaña con rutas de hiking

### Nordkette vs Patscherkofel (regla simple)

| Opción | Para qué es mejor | Pros | Contras |
|---|---|---|---|
| **Nordkette (Seegrube / Hafelekar)** | “WOW inmediato” + miradores | Está literalmente **pegado a la ciudad**, vistas brutales rápido | Más “atracción/mirador” que hiking largo; suele ser más caro |
| **Patscherkofel** | **Hiking tranquilo** (Zirbenweg) + naturaleza | Senderos más “paseo alpino”, menos sensación turística | Requiere más logística (bus + teleférico), menos impacto “instantáneo” |

**Regla práctica (muy repetida):**
- Si hay cielo claro y quieren una sola subida: **Nordkette**.
- Si el clima está “ok” y quieren caminar lindo sin matar piernas: **Patscherkofel + Zirbenweg**.

### Patscherkofel: Zirbenweg (el hike que más aparece)
- Sendero panorámico a ~2000 m, “fácil” y muy escénico.
- Info oficial ruta: `https://www.innsbruck.info/en/hiking/m/the-zirbenweg-trail.html`
- Info oficial Patscherkofel (Zirbenweg): `https://www.patscherkofelbahn.at/zirbenweg/`

> Regla práctica: si hay nubes bajas/lluvia, priorizar ciudad + cafés/museos; si hay cielo claro, subir a Nordkette.

---

## Plan simple (borrador: tránsito o 1–2 noches)

### Si es solo tránsito (0 noches → Viena el 23-sept)
- Paseo corto por **Altstadt + Golden Roof** entre trenes, o subida rápida a **Nordkette** si el horario y el clima acompañan.

### Si confirman 1–2 noches
- **Día 1 (llegada desde Suiza):** Altstadt + Golden Roof + paseo por el Inn.
- **Día 2 (clima bueno):** Montaña (Nordkette *o* Patscherkofel/Zirbenweg) + salida a Viena.

### Plan B (si el clima está feo)
- **Alpenzoo** (zoológico alpino, muy buen plan corto).
- **Bergisel** (salto de esquí + mirador).
- **Castillo de Ambras** (si quieren interior/museo).
- **Swarovski Kristallwelten** (si les tienta la experiencia, pero es más “parque” que Innsbruck puro).

---

## 🔗 Fuentes oficiales (links)
- Nordkette (teleféricos): `https://nordkette.com/` (tickets en “Ticketshop”)
- Nordkette (tarifas): `https://nordkette.com/en/service/tariffs/`
- Patscherkofelbahn (precios/tickets): `https://www.patscherkofelbahn.at/en/prices/`
- Innsbruck Card (incluye Nordkette + Patscherkofel + museos + transporte IVB): `https://www.innsbruck.info/en/sightseeing/innsbruck-card.html`

## 🧠 Consenso Reddit/foros (resumen)
- **Si confirman 1–2 noches** (borrador actual = tránsito): elegir **1 subida** (Nordkette *o* Patscherkofel) según clima/visibilidad.
- **Nordkette** suele ser la opción “wow” por estar **pegada a la ciudad** (pero es más “miradores” que trekking largo).
- **Patscherkofel** suele ganar si quieren **senderos más tranquilos** (Zirbenweg) y menos sensación de “atracción turística”.
- Si piensan hacer 2-3 atracciones + transporte, **la Innsbruck Card** puede simplificar (revisar si les cierra por precio vs lo que efectivamente hagan).

## 🔎 Hilos y lecturas (links)
- Innsbruck Card (oficial + calculadora/itinerarios): `https://www.innsbruck.info/en/sightseeing/innsbruck-card.html`
- Búsqueda r/innsbruck “Nordkette vs Patscherkofel”: `https://www.reddit.com/r/innsbruck/search/?q=Nordkette%20Patscherkofel&restrict_sr=1&sort=relevance&t=all`
- Búsqueda r/innsbruck “Zirbenweg”: `https://www.reddit.com/r/innsbruck/search/?q=Zirbenweg&restrict_sr=1&sort=relevance&t=all`

