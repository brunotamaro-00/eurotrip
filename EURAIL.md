# 🎟️ Eurail — Tracker Maestro

**Pase:** Global Pass · **15 días de viaje dentro de 2 meses** · ✅ comprado ($370 con descuento, x2 personas)
**Regla:** maximizar en tramos **caros** (UK walk-up, Suiza, larga distancia) y **day trips**; comprar **suelto** lo barato (privados, regional Italia/España, buses).

> 🇬🇧 **Gran Bretaña (Inglaterra, Escocia, Gales) está incluida en el Global Pass.** Los trenes UK (Londres→York→Edimburgo) y los **day trips británicos** (Bath/Oxford/Cambridge desde Londres; Glasgow/Stirling desde Edimburgo) **pueden gastar día de pase** — son tarifas walk-up caras (£40–90), de las que más rinden. UK **no exige reserva**: subir y andar con el pase activado.

> 🪟 **Ventana del pase (2 meses) → activar ~7-8 ago (inicio del viaje, primer day trip desde Londres).** La ventana corre hasta **~7 oct** y cubre todo lo que vale la pena: UK (5–25 ago) + Ámsterdam→París + Suiza + Austria + Viena→Praga (30 sept). **Italia y España quedan fuera a propósito**: Italo/Frecce y Ouigo/AVLO/AVE salen **más baratos al contado anticipado** que quemar un día de pase. *(Fechas post-Portugal tentativas: re-confirmar al cerrar alojamiento.)*

---

## 📱 Guía de uso — Eurail Pass para principiantes

Nunca usaron la app, así que esto va paso a paso: qué es, cuándo activarla, qué hacer antes de cada viaje y qué hacer arriba del tren.

### 1. El pase es digital (Mobile Pass)

- Todo vive en la app oficial **Eurail / Interrail Rail Planner** (buscarla como "Eurail Rail Planner" en la store — no es la app genérica de trenes).
- Al comprar el pase, se vincula a una cuenta Eurail. Cada persona necesita **su propio pase y su propio celular con la app instalada** (no sirve mostrar un pase desde el celular de otro).
- Descargar la app y loguearse **con anticipación**, no el día del viaje — a veces tarda en sincronizar el pase recién comprado.

### 2. Activación — cuándo y cómo

- El pase **no se activa solo**. Hay que activarlo manualmente en la app **antes de subir al primer tren**.
- Se puede activar hasta **11 meses después de la compra**; una vez activado corre la ventana de validez (en este caso, **2 meses corridos**).
- **Cómo activar:** en la app → sección del pase → botón "Activate pass" → elegir la fecha de inicio. Se puede activar **con hasta unos días de anticipación** para una fecha futura (no hace falta estar parado en la estación).
- **Ojo:** una vez elegida la fecha de activación, **no se puede cambiar**. Por eso en este tracker la ventana está fijada a **~7-8 ago** — confirmarla unos días antes y activar ahí, no antes.
- Activar **no gasta un día de viaje** — el contador de días de uso empieza recién cuando se marca el primer "travel day" (ver abajo).

### 3. Antes de cada viaje: agregar el "Travel Day"

Esto es lo que en la práctica más se olvida y es lo más importante:

1. Abrir la app **la noche anterior o la mañana del viaje** (con internet, aunque sea una vez).
2. Ir a "My Trip" / el pase activo → botón **"Add travel day"**.
3. Seleccionar la fecha (tiene que coincidir con el día real del viaje) y el/los tramos de tren que se van a hacer ese día.
4. Ese día queda marcado como usado — **un travel day cubre todos los trenes que se tomen ese mismo día calendario**, sin límite de tramos. Por eso en el tracker se agrupan varios trenes en un mismo día (ej. Interlaken→Lucerna→Interlaken como un solo día).
5. **Hacer esto ANTES de subir al tren.** Si el revisor pide el pase y el travel day no está agregado, lo tratan como viajar sin billete.

Consejo para principiantes: agregarlo la noche antes con wifi del alojamiento, así no depende de tener señal en el andén.

### 4. Reservas — son un producto aparte del pase

- El pase da derecho a viajar, pero en varios trenes de alta velocidad/nocturnos hay que sacar además una **reserva de asiento** (obligatoria u opcional según el tren, ver tabla de arriba).
- Las reservas se compran en `eurail.com` → "Seat reservations" (o vía la app, redirige al sitio) — **son un PDF/QR aparte**, no viven "dentro" del pase.
- Comprar Rail Planner ≠ comprar reserva. Hay que guardar el PDF de la reserva (descargarlo, no solo dejarlo en el mail) porque a veces no hay señal en la estación.
- Las reservas con cupo passholder (como Ámsterdam→París, fila 4) tienen **cupo limitado y se agotan** — comprarlas apenas abra la venta (~3 meses antes), no dejarlas para último momento.

### 5. Subiendo al tren — qué mostrar

- Llevar SIEMPRE: **el pase abierto en la app** (con el travel day de ese día ya agregado) + **la reserva** (si el tren la exige) + **un documento de identidad** (pasaporte) — el pase está nominal a cada persona y a veces lo piden.
- El revisor escanea el QR del pase en la app o simplemente lo mira. En UK usualmente no hay control de QR, pero puede pedir el pase igual.
- Tener el celular con batería: conviene descargar screenshot del pase activado + reserva por si el celular se queda sin batería o sin señal, aunque la app funciona offline una vez sincronizada.

### 6. Errores comunes a evitar

- Olvidar agregar el travel day y subir "a confiar" → multa o tratado como sin billete.
- Activar el pase antes de tiempo por las dudas → se pierden días de la ventana de 2 meses sin usar.
- Confundir "comprar la reserva" con "el pase ya me cubre" en trenes de reserva obligatoria (TGV, Frecce, AVE, nocturnos) — para esos tramos el pase da el descuento/base pero **la reserva se paga y se gestiona aparte**, y en varios de los tramos post-Portugal (Italia, España) directamente conviene comprar el pasaje suelto y no gastar día de pase (ver tabla de arriba).
- No chequear la app con anticipación por si hay obras/cambios de ruta (ej. obras Arlberg en el tramo Suiza→Austria, fila 8) — la reserva opcional ayuda a asegurar asiento en esos casos.

---

## Conteo de días

- **Firmes (alto valor, casi seguros): 9 / 15** — Bath, Londres→York, York→Edimburgo, Ámsterdam→París, Friburgo→Interlaken, 2 day trips suizos, Interlaken→Viena, Viena→Praga.
- **Comodines flex (F1-F4): 4** — day trip Oxford/Cambridge, 4º día suizo (o Escocia), 3er day trip Londres, Innsbruck→Viena si se duerme ahí.
- Con el apetito elegido (**day trips UK + Suiza a full**), llegar a 15 es **holgado**; tomando 3 de los 4 comodines ya estás en **12**.
- Tramo post-Portugal **tentativo**: las asignaciones de sept en adelante son candidatas, no firmes.

---

## 🗓️ Asignación de los 15 días (9 firmes + 4 comodines flex)

| # | Día (tent.) | Tramo / uso del día | ¿Reserva? | Por qué rinde |
|---|-------------|---------------------|-----------|----------------|
| 1 | ~8 ago | Day trip Londres → **Bath** (GWR) | No | walk-up i/v ~£50-60 |
| 2 | 13 ago | **Londres → York** (LNER) | No | walk-up ~£90 (advance £23) |
| 3 | 15 ago | **York → Edimburgo** (LNER) | No | walk-up ~£70 |
| 4 | 29 ago | **Ámsterdam → París** (Eurostar) | ✅ €22-25 (cupo limitado) | vs €35 advance; sumar día ya que sobran |
| 5 | 19 sep | **Friburgo → Interlaken** (ICE+SBB) | No | ~€40-50; mismo día cubre Interlaken→Lauterbrunnen/Grindelwald |
| 6 | 20 sep | Day trip suizo (Lucerna / Berna / Oeschinensee) | No | trenes SBB carísimos |
| 7 | 21 sep | Day trip suizo | No | ídem |
| 8 | 23 sep | **Interlaken → (Innsbruck) → Viena** (Railjet) | opc. ~€3 | el día más valioso, ~CHF 90-130 ⚠️ obras Arlberg 14 jun–14 oct: vía St. Gallen |
| 9 | 30 sep | **Viena → Praga** (Railjet) | opc. ~€3 | cubierto; marginal pero suma |
| **F1** | flex | **Day trip Londres → Oxford o Cambridge** (~10 ago) | No | ~£28-35 i/v; comodín para llegar a 15 |
| **F2** | flex | **4º día suizo** (22 sep) *o* Edimburgo→Glasgow/Stirling | No | Suiza alto valor; Escocia barato pero gratis con pase |
| **F3** | flex | Day trip Londres #3 (Windsor/Cambridge) | No | walk-up |
| **(F4)** | flex | **Innsbruck → Viena 25 sep** — SOLO si se duerme en Innsbruck (parte el día 8 en dos) | opc. | larga distancia |

> **9 días firmes + 4 comodines flex (F1-F4) = hasta 15.** Tomando 3 de los 4 comodines llegás a 12; con los day trips UK extra y el 4º suizo, a 15 holgado.
> **Innsbruck (abierto):** si es **paso de largo** (0 noches) → el día 8 es Interlaken→Viena directo = **1 día**. Si se **duerme 1-2 noches** → Interlaken→Innsbruck (23 sept) + Innsbruck→Viena (25 sept) = **2 días** (fila F4 activa).

---

## Tramos candidatos (tabla de referencia)

| # | Tramo | Fecha (tent.) | ¿Reserva oblig.? | Costo suelto | ¿Usar día de pase? | Estado |
|---|-------|---------------|------------------|--------------|--------------------|--------|
| — | **Day trips Londres** (Bath, Oxford, Cambridge, Windsor) | 5–13 ago | ❌ UK sin reserva | £25-60 i/v walk-up | ✅ **SÍ** (walk-up caro) | tentativo |
| 1 | **Londres → York** | 13 ago | ❌ UK sin reserva | walk-up ~£90 / advance £23 | ✅ **SÍ** | fijo |
| 2 | **York → Edimburgo** | 15 ago | ❌ UK sin reserva | walk-up ~£70 | ✅ **SÍ** | fijo |
| — | Day trips Edimburgo (Glasgow/Stirling/St Andrews) | 15–18 ago | ❌ UK sin reserva | £10-20 i/v | 🟡 flex (barato, pero gratis con pase) | tentativo |
| — | **Edimburgo → Highlands** | 18 ago | — | auto ($150–220 ÷2) | ❌ **NO (auto, no tren)** | ✅ auto confirmado |
| — | Highlands → Ámsterdam | 25 ago | — | vuelo $92 | ❌ NO (vuelo) | ✅ comprado |
| 3 | **Ámsterdam → París** | 29 ago | ✅ Eurostar (cupo passholder) | €22-25 reserva / €35 advance | ✅ **SÍ** (sobran días; reserva < advance) | fijo |
| — | ~~París → Estrasburgo~~ (**vuelo París→Lisboa** 4 sept + **vuelo Porto→Estrasburgo** 12 sept) | 4 / 12 sept | — (vuelos) | $85 + $90 | ❌ NO (vuelos) | ✅ comprados |
| 4 | Estrasburgo → Colmar → Friburgo | 14 / 16 sept | ❌ regional/bus | €7–12 · €27–40 ó Regio-Elsass €37/2-5p | 🟡 marginal (regional barato; solo si sobra día) | tentativo |
| 5 | **Friburgo → Interlaken** (Suiza) | 19 sept | ❌ ICE+SBB sin reserva | ~€39–50 | ✅ **SÍ** (entrada Suiza + interno mismo día) | tentativo |
| 6 | Day trips suizos (Lucerna/Berna/Oeschinensee) | 20–22 sept | ❌ SBB sin reserva | €30-50+ | ✅ **SÍ** (SBB carísimo) | tentativo |
| 7 | **Interlaken → (Innsbruck) → Viena** | 23 sept | ❌ Railjet (reserva opc. €3) | ~CHF 90–130 | ✅ **SÍ** (el más valioso) ⚠️ obras Arlberg | tentativo |
| 8 | **Viena → Praga** | 30 sept | Railjet reserva opc. ~€3 | €14.90 antic. | 🟡 marginal (cubierto; suma para llegar a 15) | tentativo |
| 9 | Praga → Cracovia | 3-5 oct | privados, asiento asignado | €13–16 (Leo Express/RegioJet) | ❌ **NO (privados, fuera de Eurail)** | suelto |
| 10 | Cracovia → Budapest | 7-10 oct | ❌ (RegioJet/FlixBus aparte) | €19–45 | ❌ **NO** (privado/bus, fuera de Eurail) | tentativo |
| 11 | Budapest → Liubliana | 11-14 oct | ❌ | FlixBus directo €19–45 | ❌ **NO** (bus directo gana) | tentativo |
| 12 | Liubliana → Trieste → Florencia | 15 oct | parcial (Frecce/Italo) | loop AUTO + Italo/Frecce advance | ❌ **NO** (fuera de ventana + Italo barato) | tentativo |
| 13 | Florencia → Roma → Nápoles | 20–27 oct | ✅ Italo/Frecce | $11–28 advance | ❌ NO (fuera de ventana + Italo barato) | suelto |
| 14 | Barcelona → Madrid | 13 nov | ✅ AVE/Ouigo | $17–41 (Ouigo/AVLO) | ❌ NO (fuera de ventana + low-cost) | suelto |

**Otros day trips a evaluar:** **Bratislava** (desde Viena — el **EURegio "Bratislava Ticket" €18** i/v incluye transporte urbano y **gana** a gastar día de pase → comprar EURegio), **Pompeya/Herculano** y **Montserrat** quedan **fuera de la ventana** (Italia/España oct-nov) → suelto.

---

## Reservas obligatorias (resumen)

- **Eurostar Ámsterdam → París (29 ago):** cupo passholder **limitado** y se agota; reservar apenas abra (~3 meses antes). `https://www.eurail.com/en/plan-your-trip/seat-reservations`
- **UK (Londres→York, York→Edimburgo, day trips):** **sin reserva** — subir y andar con el pase.
- **Railjet (Suiza→Austria, Viena→Praga):** reserva **opcional** ~€3 (recomendada en tramos llenos).
- **TGV inOui / Italo / Frecciarossa / AVE:** reserva obligatoria — pero quedan **fuera de la ventana o más baratos sueltos**; no se cubren con el pase.
- **Trenes nocturnos (ÖBB Nightjet):** reserva de litera/cama obligatoria; abren ~3 meses antes (no hay nocturnos asignados en este plan).

---

## Estrategia

- **Gastar los 15 días donde el tren es caro:** bloque **UK** (Londres→York, York→Edimburgo + day trips Bath/Oxford/Cambridge walk-up) y **Suiza** (Friburgo→Interlaken + day trips internos + Interlaken→Viena). En Suiza la **montaña va aparte** (teleféricos/JB con −25% Eurail, no cubiertos).
- **Comprar suelto lo barato o privado:** Portugal (fuera de ventana), Praga→Cracovia y Cracovia→Budapest (Leo Express/RegioJet/FlixBus **privados, fuera de Eurail**), Budapest→Liubliana (FlixBus), Italia (Italo/Frecce advance, fuera de ventana), España (Ouigo/AVLO, fuera de ventana), Bratislava (EURegio €18).
- **Ventana 2 meses = activar al inicio (~7-8 ago).** El último tramo in-pass valioso es Viena→Praga (30 sept), muy dentro de la ventana. No tiene sentido empujar la ventana hacia Italia/España (valor casi nulo ahí).

---

> Fuente base: sección Transporte de [`PRESUPUESTO.md`](PRESUPUESTO.md) + investigación Eurail (eurail.com, seat61.com, 2026). Actualizado por la skill `/transporte` (último: Lisboa, Porto, Friburgo, Interlaken, Innsbruck, Viena, Praga).
